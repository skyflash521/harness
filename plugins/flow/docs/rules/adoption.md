...
参照し、必須条項を起動時に機械確認する([起動時の契約確認](#起動時の契約確認))。
guard プラグインはこの契約を課さない(有効化だけを各リポジトリの `enabledPlugins` で行う)。

条項3・4の登録先は、そのリポジトリの**追跡される** `.claude/settings.json` とする。共有されない
設定では条項を満たしたことにならない。
...
ドキュメントの置き場とする。先頭にスラッシュを付けない `.scratch/` だけのパターンは場所を問わず一致するので、これ1行で
...
`sandbox.excludedCommands` の全エントリを登録する。
...
4. **条項3**: marketplace を登録して flow を導入する。この操作が `enabledPlugins` を書き込む。
   ...
   **導入(`install`)ではプロジェクトスコープを選ぶ。** 既定はユーザースコープである。書き込まれた
   先が違えば次の検証が落ちるので、そのときは書き込まれた側の登録を外してやり直す。marketplace の
   登録にスコープを指定しないのは、登録先がマシン側の状態だからで、プロジェクトスコープを選ぶと
   追跡される設定にキーが書かれるが、それだけではプラグインは取得も導入もされない。
