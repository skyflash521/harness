...
条項3・4の登録先は、そのリポジトリの**追跡される** `.claude/settings.json` とする。clone した
環境でそのまま成立する必要があるため、共有されない設定では条項を満たしたことにならない。

**ユーザースコープでの有効化は使わない。** flow のフックは有効なリポジトリすべてで発火するので、
全プロジェクトへ適用すると、契約を満たしていないリポジトリにもコミットゲートの規則が及ぶ。
...
4. **条項3**: marketplace を登録して flow を導入する。この操作が `enabledPlugins` を書き込む。
   ...
   - `claude plugin marketplace add <harness の所在> --scope project` と
     `claude plugin install <プラグイン>@harness --scope project`。ターミナルから実行する。

   **どちらの経路でもプロジェクトスコープを選ぶ。** 既定はユーザースコープである。実行後、
   `enabledPlugins` と marketplace の登録の**両方**が追跡される `.claude/settings.json` に入って
   いることを確かめる。marketplace の登録だけユーザースコープに残ると、clone した環境では
   `flow@harness` が指す所在が無く、解決できない参照になる。入っていなければ、書き込まれた側の
   登録を外して CLI 経路でやり直す。ユーザースコープに残った登録はこのリポジトリのどのファイルにも
   現れず、以後どの検査でも捕捉されない。
...
[条項3](#3-有効化)はこのスクリプトで確認しない。スキルが起動できていれば有効化されてはいるが、
それが登録先の要求まで満たすかは起動から分からない。登録先は、`enabledPlugins` と marketplace の
登録が追跡される `.claude/settings.json` に入っていることを[導入](#導入手順)の時点で確かめて
担保する。
